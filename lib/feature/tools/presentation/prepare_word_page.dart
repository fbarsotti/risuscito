...
import 'package:risuscito/feature/tools/presentation/word_song_picker_page.dart';
import 'package:url_launcher/url_launcher.dart';

class PrepareWordPage extends StatefulWidget {
  const PrepareWordPage({Key? key}) : super(key: key);

  @override
  State<PrepareWordPage> createState() => _PrepareWordPageState();
}

class _PrepareWordPageState extends State<PrepareWordPage> {
  ...
  final Map<String, SongDomainModel?> _selectedSongs = {
    'word_entry_song': null,
    'word_reading_1': null,
    'word_reading_2': null,
    'word_reading_3': null,
    'word_final_song': null,
  };
  ...
  Future<void> _shareOnWhatsApp() async {
    ...
    buffer.writeln('*${loc.translate('word_share_title')}*');
    buffer.writeln();
  ...
  Future<void> _pickSong(String momentKey) async {
    ...
    final song = await Navigator.of(context).push<SongDomainModel>(
      CupertinoPageRoute(
        builder: (context) => WordSongPickerPage(
  ...
  Widget build(BuildContext context) {
    ...
    return CupertinoPageScaffold(
      navigationBar: CupertinoNavigationBar(
        middle: Text(loc.translate('prepare_word')!),
